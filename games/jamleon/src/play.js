var playState = {
	...
	worldCreate: function() {
		...
		this.level = 1;
		this.lastLevel = 3;
		this.faseForeground = game.add.image(0, 0, 'level_' + this.level + '_foreground');
	},
	collision: function() {
	...
	playerCreate: function() {
		...
		this.jump = game.add.audio('player_jump');
	...
	gaviometroCreate: function() {
		...
		this.gaviometroBar = game.add.image(36, game.world.height - 44, 'hawk_bar');
		game.add.tween(this.gaviometroBar.scale)
			.to({
				x: 10
			}, 5)
			.start();
		this.gaviometroImage = game.add.image(16, game.world.height - 64, 'hawk_icon');
		this.shadow = game.add.image(game.world.width + 100, 0, 'hawk_shadow');
	},
	...
	nextLevel: function() {
		game.state.start('level_' + this.level);
	},
	victory: function() {
		//this.game.time.events.delay(1000, this.goToMenu, this);
		this.victorySound = game.add.audio('victory');
		this.hasWon = true;
		this.game.add.text(game.camera.width / 2, game.camera.height / 2, 'Boa! Próxima fase!', {
			font: 'Arial 128px',
			fill: '#ffffff'
		});
