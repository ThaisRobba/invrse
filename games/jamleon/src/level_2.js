var level_2 = {
	...
	worldCreate: function() {
		...
		this.level_Background = game.add.image(0, 0, 'level_' + this.level +
			'_background');
		this.level_Foreground = game.add.image(0, 0, 'level_' + this.level +
			'_foreground');
	},
	collision: function() {
	...
	playerCreate: function() {
		this.canWin = true;
		this.player = game.add.sprite(128, game.world.height - 178, 'player');
	...
	playerColorChange: function() {
		...
		this.player.color = colors[game.rnd.integerInRange(0, 1)];
	},
	...
	playerDie: function() {
		...
		this.deathLabel = game.add.text(game.camera.width / 2, game.camera.height / 2,
			'X_X você morreu.', {
				font: '32px Arial',
				fill: '#ffffff'
			});
